...
- **Reopening shipment questions**: `isIcegridTable` detects ICEGATE customs columns; `RightRibbon.svelte` generically surfaces a dedicated `help-circle` button (`onOpenQuestions`) when active. `reopenIcegridConfirmation` re-evaluates declarations/rates over table data and pushes an undoable table update.
